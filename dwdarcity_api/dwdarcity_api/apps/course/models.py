...
class Course(BaseModel):
    ...
    price = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="课程原价", default=0,
                                help_text='如果这里填写的价格为0,则表示当前课程被购买时，没有永久有效的期限')
    ...
    def lessons_list(self):
        """展示课程列表中推荐的4个课程信息"""
        data_list = []
        lesson_list = CourseLesson.objects.filter(is_show=True, is_delete=False, course_id=self.id, is_show_list=True)
    ...
    def discount_name(self):
        ...
        if len(activity_list) > 0:
            """当前课程有参与1个以上活动时，才有优惠累类型数据"""
            # 获取第一个活动
            active = activity_list[0]
            # 获取该活动的优惠策略名称
    ...
    def real_price(self, expire_id=0):
        ...
        active_list = self.activity_list()
    ...
    def activity_time(self):
        ...
        if len(activity_list) > 0:
            ...
            now_time = datetime.now().timestamp()  # 当前时间
            end_time = activity.active.end_time.timestamp()
...
class CourseLesson(BaseModel):
    ...
    course = models.ForeignKey(verbose_name='关联课程', to="Course", related_name='course_lesson', on_delete=models.CASCADE)
    is_show_list = models.BooleanField(verbose_name="是否推荐到课程列表中显示", default=False)
    lesson = models.IntegerField(verbose_name="第几课时")

    class Meta:
        db_table = "dw_course_lesson"
        verbose_name = "课程课时"
...
class CourseDiscountType(BaseModel):
    ...
    class Meta:
        db_table = "dw_course_discount_type"
        verbose_name = "课程优惠类型"
...
class CoursePriceDiscount(BaseModel):
    ...
    class Meta:
        db_table = "dw_course_price_dicount"
...
class CourseExpire(BaseModel):
    ...
    course = models.ForeignKey(to='Course', related_name='course_expire', on_delete=models.CASCADE, verbose_name='课程名称')
    expire_time = models.IntegerField(verbose_name='有效期', null=True, blank=True, help_text='有效期按天数计算')
    expire_text = models.CharField(verbose_name='有效期提示文本', max_length=64, null=True, blank=True)
    price = models.DecimalField(max_digits=6, decimal_places=2, verbose_name='课程价格', default=0)

    class Meta:
        db_table = 'dw_course_expire'
        verbose_name = '课程有效期选项'
        verbose_name_plural = verbose_name

    def __str__(self):
        return "课程:%s,有效期:%s,价格:%s," % (self.course, self.expire_time, self.price)
